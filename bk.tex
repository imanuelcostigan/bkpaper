...
perhaps at issuer default} hedging strategy associated with the
...
\sidenote{Even if we use the hedging strategy B\&K specify I will demonstrate in a later
section that it is impossible to arrive at B\&Ks PDE.}
...
\begin{IEEEeqnarray}{rCl}
  Z dt & = & \left(\partial_t \hat{V} + \frac{1}{2}\sigma^2 S^2 \partial^2_S\hat{V}
    ...
    & & \quad \left. {} + (r_C - q_C) (g_C - \hat{V}) + r P + \lambda_B(P - P_D) - r_X X\right.\bigg) dt \label{eq:zterm}\\
    & = & \left(\partial_t \hat{V} + \mathcal{A}_t\hat{V} + \lambda_C (g_C - \hat{V}) \nonumber\right.\\
    & & \quad \left. {} - r (\hat{V} + X) - \lambda_B(\hat{V} + X + P_D) - r_X X\right) dt\nonumber
\end{IEEEeqnarray}

Denoting $\tilde{\epsilon}_h = g_B + P_D + X$ we find that:

\begin{IEEEeqnarray}{rCl}
  d\hat{V} + d\Pi & = & Y_B dJ_B + Z dt \nonumber \\
  & = & (\Delta\hat{V}_B -(P - P_D))dJ_B \nonumber\\
    & & \:+ \left(\partial_t \hat{V} + \mathcal{A}_t\hat{V} + \lambda_C (g_C - \hat{V}) \nonumber\right.\\
    & & \quad \left. {} - r (\hat{V} + X) - \lambda_B(\hat{V} + X + P_D) - r_X X\right) dt\nonumber\\
    & = &  (g_B - \hat{V} + (\hat{V} + X + P_D))dJ_B \nonumber\\
    & & \:+ \left(\partial_t \hat{V} + \mathcal{A}_t\hat{V} + \lambda_C (g_C - \hat{V}) \nonumber\right.\\
    & & \quad \left. {} - r (\hat{V} + X) - \lambda_B(\hat{V} + X + P_D) - r_X X\right) dt\nonumber\\
...
\begin{IEEEeqnarray}{rCl}
  ...
  0 & = & \partial_t \hat{V} + \mathcal{A}_t\hat{V} - (r + \lambda_C + \lambda_B) \hat{V} \right. \nonumber \\
